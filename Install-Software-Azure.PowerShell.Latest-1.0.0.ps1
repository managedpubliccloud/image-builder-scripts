...
$AppVersion     = "Latest"
$AppSetupFile   = "Az-Cmdlets-*-x64.msi"   # GitHub release asset filter (wildcard), not a fixed filename
$Params         = "/quiet /norestart"
$TestPaths      = @(
                    "$env:ProgramFiles\WindowsPowerShell\Modules\Az"
...
Function MainScript {
    ...
    # GitHub process: resolve the latest stable Azure PowerShell (Az cmdlets) x64 MSI each run
    $URIRoot = "https://github.com/Azure/azure-powershell/releases/latest"
    Write-LogEntry -Message "URI Root (GitHub): $URIRoot" -Level Info

    $AppURI = Get-GitHubLatestLink -Repo "Azure/azure-powershell" -AssetFilter $AppSetupFile -ExcludeFilter "*preview*"
    if (-not $AppURI) {
        $msg = "Failed to resolve GitHub latest asset for Azure/azure-powershell (filter: $AppSetupFile)"
        Write-LogEntry -Message $msg -Level Error -EventLog $true
        Exit 1
    }

    $AppSetupFile = ([System.Uri]$AppURI).Segments[-1]
...
function Get-GitHubLatestLink {
    ...
    try {
        Write-Verbose -Message "$($MyInvocation.MyCommand.Name): Querying GitHub API for latest release of $Repo"
        $release = Invoke-RestMethod -Uri $apiUrl -Headers @{ "User-Agent" = "PowerShell" } -ErrorAction Stop

        $assets = @($release.assets)
        if (-not $assets -or $assets.Count -eq 0) {
            Write-Error "No assets returned from GitHub for '$Repo' latest release."
            return $null
        }

        $asset = $assets | Where-Object { $_.name -like $AssetFilter }
        if ($ExcludeFilter) {
            $asset = $asset | Where-Object { $_.name -notlike $ExcludeFilter }
        }
        $asset = $asset | Select-Object -First 1

        if (-not $asset) {
            Write-Error "No asset matching '$AssetFilter' found in the latest release of $Repo."
            return $null
        }
        ...
        Write-Error "Failed querying GitHub API for $Repo latest release: $($_.Exception.Message)"
        return $null
    }
}
