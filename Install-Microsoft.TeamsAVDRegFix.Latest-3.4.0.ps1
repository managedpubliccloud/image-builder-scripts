<#
.Synopsis
   MAVD OpsScript Remediation - Microsoft Teams AVD Registry Fix
.DESCRIPTION
   Checks the registry for the Teams AVD mode setting and creates/corrects it if missing or incorrect:
       - HKLM:\SOFTWARE\Microsoft\Teams\IsWVDEnvironment = 1 (DWORD)
   Verifies the fix
   Updates local software register with status
   Adapted from Template-Install-Software-Publisher.App.Version-1.0.0.ps1 - no download/install step,
   since this is a registry remediation rather than a package installer.
...
# Remediation target
$TargetKeyPath   = "HKLM:\SOFTWARE\Microsoft\Teams"
$TargetValueName = "IsWVDEnvironment"
$TargetValue     = 1
...
Function MainScript {
    ...
    Write-LogEntry "ScriptType: $ScriptType CustID: $CustID TestLab: $TestLab" -Level Info

    # Ensure target key exists
    try {
        if (!(Test-Path $TargetKeyPath)) {
            Write-LogEntry "$TargetKeyPath does not exist, creating it" -Level Info
            New-Item -Path $TargetKeyPath -Force -ErrorAction Stop | Out-Null
        } else {
            Write-LogEntry "$TargetKeyPath already exists" -Level Info
        }
    }
    catch {
        $StatusMessage = "Failed to create $TargetKeyPath : $($_.Exception.Message)"
    ...
    # Check current value and only write if missing or incorrect
    try {
        $currentValue = (Get-ItemProperty -Path $TargetKeyPath -Name $TargetValueName -ErrorAction SilentlyContinue).$TargetValueName

        if ($null -eq $currentValue) {
            Write-LogEntry "$TargetValueName is missing, setting Teams to AVD mode" -Level Info -EventLog $true
            New-ItemProperty -Path $TargetKeyPath -Name $TargetValueName -PropertyType DWORD -Value $TargetValue -Force -ErrorAction Stop | Out-Null
        }
        elseif ($currentValue -ne $TargetValue) {
            Write-LogEntry "$TargetValueName is set to $currentValue, correcting to $TargetValue" -Level Warning -EventLog $true
            New-ItemProperty -Path $TargetKeyPath -Name $TargetValueName -PropertyType DWORD -Value $TargetValue -Force -ErrorAction Stop | Out-Null
        }
        else {
            Write-LogEntry "$TargetValueName is already set to $TargetValue, no change required" -Level Info
        }
    }
    catch {
        $StatusMessage = "Failed to set $TargetKeyPath\$TargetValueName : $($_.Exception.Message)"
        Write-LogEntry $StatusMessage -Level Error -EventLog $true
        $result = Update-PackageRegistryStatus -RegistryPath $RegistryPath -Success $false -StatusMessage $StatusMessage -ScriptVersion $ScriptVersion -AppPublisher $AppPublisher -AppName $AppName -AppVersion $AppVersion -ScriptType $ScriptType
        Write-LogEntry -Message $result.Message -Level Info
        if ($ScriptType -eq "Public") {
    ...
    # Test paths to verify successful remediation
    ...
    if ($result.Success) {
        ...
        if ($AppPaths.AllMatch) {
            # Update registry with remediation status
            $StatusMessage = "Remediation successful, and $($AppPaths.Message)"
            ...
            # Update registry with remediation failure status
            $StatusMessage = "Remediation ran, but verification failed: $($AppPaths.Message)"
            $result = Update-PackageRegistryStatus -RegistryPath $RegistryPath -Success $false -StatusMessage $StatusMessage -ScriptVersion $ScriptVersion -AppPublisher $AppPublisher -AppName $AppName -AppVersion $AppVersion -ScriptType $ScriptType
    ...
    $Message = "$TargetValueName remediation complete. Duration: $duration seconds"
...
$LogPath = $env:LOCALAPPDATA        # Specify log output folder - resolves under SYSTEM profile when run during image build
