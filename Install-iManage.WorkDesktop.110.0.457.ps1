<#
.SYNOPSIS
    iManage.WorkDesktop.110.0.457 installation script
.DESCRIPTION
    This script downloads and installs iManage.WorkDesktop.110.0.457 on a Windows machine.
.NOTES
...
$AppName = "WorkDesktop"
$AppVersion = "110.0.457"
$AppSetupFile = "iManageWorkDesktopforWindowsx64.exe"
$AppSupportFile = "imanage-desktop-setup.iss"
$ExeParams = "/s imanage-desktop-setup.iss"
...
$AppURI = "$URIRoot/$AppPublisher.$AppName.$AppVersion/$AppSupportFile"
$AppURI = $AppURI.ToLower()
$SetupFolderSupportFile = Join-Path -Path $RootFolderApp -ChildPath $AppSupportFile
Write-Host "Downloading supporting file $AppSupportFile from $AppURI to $SetupFolderSupportFile"
try {
    $ProgressPreference = 'SilentlyContinue'
    Invoke-WebRequest -Uri $AppURI -OutFile "$SetupFolderSupportFile" -UseBasicParsing -ErrorAction Stop
    Write-Host "Downloaded $AppMoniker $AppSupportFile from $AppURI to $SetupFolderSupportFile"
}
catch {
    Write-Host "Failed to download $AppMoniker $AppSupportFile from $AppURI to $SetupFolderSupportFile"
    Exit 1
}
