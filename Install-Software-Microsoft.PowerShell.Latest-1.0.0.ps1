...
$AppName        = "PowerShell"
$AppVersion     = "Latest"
$AppSetupFile   = "PowerShell-7*-win-x64.msi"
...
Function MainScript {
    ...
    # GitHub process: resolve the latest stable PowerShell 7.x win-x64 MSI each run
    $URIRoot = "https://github.com/PowerShell/PowerShell/releases/latest"
    Write-LogEntry -Message "URI Root (GitHub): $URIRoot" -Level Info

    $AppURI = Get-GitHubLatestLink -Repo "PowerShell/PowerShell" -AssetFilter $AppSetupFile -ExcludeFilter "*preview*"
    if (-not $AppURI) {
        $msg = "Failed to resolve GitHub latest asset for PowerShell/PowerShell (filter: $AppSetupFile)"
