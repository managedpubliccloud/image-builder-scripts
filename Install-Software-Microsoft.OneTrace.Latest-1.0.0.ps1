...
$TestPaths      = @(
                    "C:\Program Files (x86)\Configuration Manager Support Center"
...
Function MainScript {
    ...
    $regCheckResult = Get-RegistryKeyInfo -KeyPath $RegistryPath
    $isAlreadyInstalled = $false
    if ($regCheckResult.Success -and $regCheckResult.Data.PropertyDetails['Status'].Value -eq "INSTALLED") {
        $isAlreadyInstalled = $true
        Write-LogEntry "$AppMoniker is already installed. Proceeding with uninstall first." -Level Info
    }

    # Do uninstall if already installed (MSI files only)
    $fileExtension = [System.IO.Path]::GetExtension($AppSetupFile).ToLower()
    if ($isAlreadyInstalled -and $fileExtension -eq ".msi") {
        Write-LogEntry "Uninstalling existing $AppMoniker from $SetupFolderFile..." -Level Info
        $uninstallArgs = "/x `"$SetupFolderFile`" /quiet /norestart"
        Write-LogEntry "Executing Start-Process -FilePath msiexec.exe -ArgumentList $uninstallArgs -Wait -Passthru" -Level Info
        $UninstallResult = Start-Process -FilePath msiexec.exe -ArgumentList $uninstallArgs -Wait -Passthru
        
        if ($UninstallResult.ExitCode -eq 0) {
            Write-LogEntry "Uninstallation of $AppMoniker completed with exit code 0" -Level Info
            # Get current registry values and update with UNINSTALL status
            ...
            # Set StatusMessage
            $StatusMessage = "Uninstallation successful"
            ...
            if ($setResult.Success) {
                Write-LogEntry -Message "Successfully updated registry with Status=UNINSTALL, StatusDate=$dateTimeNow" -Level Info
            } else {
                Write-LogEntry -Message "Failed to update registry with UNINSTALL status: $($setResult.Message)" -Level Error
            }
        } else {
            $StatusMessage = "Uninstallation of $AppMoniker failed with exit code $($UninstallResult.ExitCode)"
            Write-LogEntry $StatusMessage -Level Error -EventLog $true
            # Update registry with UNINSTALL status but mark as failure in message
            $regResult = Get-RegistryKeyInfo -KeyPath $RegistryPath
            if ($regResult.Success) {
                $propertyDetails = $regResult.Data.PropertyDetails
            } else {
    ...
    if ($fileExtension -eq ".msi") {
        $arglist = "/i `"$SetupFolderFile`" $Params"
...
function Get-PackageURIRoot {
    ...
    .EXAMPLE
        $result = Get-PackageURIRoot -ScriptType "Public"
        if ($result.Success) {
            $uriRoot = $result.Data
        }
    #>
...
function Start-Logging {
    ...
    Try {
       if (Test-Path -Path $LogFilePath){
        ...
        Add-Content -Path $LogFilePath -Value "Version:`t`t $Version"
